...
//            Log.i("dateIntervention", dateIntervention)
        ...
        btn_save.setOnClickListener {
            ...
            context?.let {
                ...
                val mIntervention = Intervention(1,dateIntervention,plombier,typeIntervention)

                if (intervention == null)
                {
                    Helpers.saveInterventionToJson(requireContext(),mIntervention)
                    it.toast("Intervention Saved")

                } else {
                    mIntervention.id = intervention!!.id
                    Helpers.updateInterventionToJson(requireContext(),mIntervention)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when(item.itemId) {
            R.id.delete -> if (intervention != null) deleteInter() else context?.toast("Cannot Delete")
        }
    ...
    private fun deleteInter()
    {
        AlertDialog.Builder(context).apply {
            ...
            setPositiveButton("yes"){ _ , _ ->
                    ...
                    Helpers.deleteIntervention(context,intervention!!)
                    val action = AddInterventionFragmentDirections.actionSaveIntervention()
                    Navigation.findNavController(requireView()).navigate(action)
